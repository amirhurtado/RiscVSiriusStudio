...
export function SwitchAutoFocusOnNewLine() {
  const { switchAutoFocusOnNewLine, setSwitchAutoFocusOnNewLine } = useCustomOptionSimulate();

  const handleCheckedChange = (value: boolean) => {
    setSwitchAutoFocusOnNewLine(value);
  };
  ...
  return (
    <div className="flex items-center space-x-2">
      <SwitchPrimitive.Root
        data-slot="switch"
        checked={switchAutoFocusOnNewLine}
      ...
      <Label htmlFor="airplane-mode" className="text-md">
        Move focus to each new program line automatically.
      </Label>
    </div>
  );
}
export default SwitchAutoFocusOnNewLine;
