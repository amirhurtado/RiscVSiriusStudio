...
import { template, capitalize } from "lodash-es";
import {
  usesRegister,
  usesALU,
  writesRU,
...
function styleSignals(cpuData: SimulatorInfo, instType: string, style: string) {
  const {
    cpuElements: {
      ...
      SgnDMCTRLPTH: DmCtrlSignal,
      SgnDMWRPTH: DmWrSignal,
    },
  } = cpuData.getInfo();
  const instruction = cpuData.getInstruction();
  const applyStyle = (signal: HTMLElement) => {
  ...
  switch (instType) {
    ...
    case "U":
      if (isAUIPC(instruction.type, instruction.opcode)) {
        ...
        // LUI:
        [ALUBSignal, RUWrSignal, RUDataWrSrcSignal, ImmSrcSignal].forEach(
          applyStyle
        );
      }
      [
...
function immTooltipText(cpuData: SimulatorInfo) {
  ...
  const immLength = `${immInfo.bits} bits.`;
  const immValue2 = `${immInfo.value2}.`;
  const immValue10 = `${immInfo.value10}.`;
  const instPosition = `${immInfo.instruction} bits in instruction.`;
  const immPosition = `${immInfo.imm} bits in immediate.`;
